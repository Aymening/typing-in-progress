...
func Add_comment(w http.ResponseWriter, req *http.Request) {
	...
	isLoged, user_id, err := forum.IsLoggedIn(req, "token")
	if err != nil{
	...
	cmd := `INSERT INTO comment (post_id, user_id,comment_content,created_at,hasChange) VALUES(?, ?, ?, ?, 0)`
	_, errr := forum.ExecQuery(cmd, post_id, user_id, content, time.Now(), 0)
	if errr != nil {
		jsonResponse(w, http.StatusInternalServerError, "Something wrong")
		return
	}
